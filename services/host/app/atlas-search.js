...
export class AtlasSearch{
  constructor(ui){
    ...
    mount.innerHTML=`<form class="atlas-search-bar" role="search"><button class="atlas-icon" type="submit" aria-label="인물·사건 찾기">${icon('search')}</button><input id="atlasQuery" type="search" autocomplete="off" placeholder="인물·사건 찾기" aria-label="인물·사건 찾기" aria-controls="atlasSearchResults"><button class="atlas-icon atlas-search-clear" type="button" aria-label="검색어 지우기" hidden>${icon('close')}</button></form>`;
  ...
  render(){
    ...
    this.pane.querySelector('.atlas-search-note').textContent=this.ui.chronicle.error||(this.ui.chronicle.loading?'자료를 불러오고 있습니다…':query?`고른 사료 기준 ${rows.length}개`:'인물이나 사건 이름을 입력해 보십시오.');
    this.pane.querySelector('.atlas-search-results').innerHTML=rows.slice(0,40).map(({entity,related})=>`<button class="atlas-result${entity.id===this.selected?' selected':''}" data-search-entity="${esc(entity.id)}" aria-pressed="${entity.id===this.selected}">${icon(entity.type==='Person'?'person':'event')}<span><strong>${esc(data.label(entity))}</strong><small>${related?'관련 ':''}${typeName(entity.type,entity)}${data.labelNote(entity)?' · '+esc(data.labelNote(entity)):''} · ${esc(data.datesLabel(entity.id))}</small></span>${icon('right')}</button>`).join('')||(query&&!this.ui.chronicle.loading?'<p class="atlas-muted">검색 결과가 없습니다. 다른 이름을 쓰거나 사료를 바꿔 보십시오.</p>':'');
    ...
    detail.innerHTML=`<h2>${esc(data.label(entity))}</h2>${description?`<p class="atlas-description">${esc(description)}</p>`:''}
      ...
      ${claim?`<button class="atlas-source-link" data-search-proof>${esc(claim.sourceLabel||'출처 보기')} ↗</button>`:''}
