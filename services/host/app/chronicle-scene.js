...
export class ChronicleScene {
  ...
  activity(id){
    ...
    const row=this.assets.rows.find(r=>r.sceneId===selected.id&&r.kind==='event')
      ||this.assets.rows.find(r=>r.sceneId===selected.id&&r.kind==='person');
    const claimIds=[...new Set([...selected.claimIds,...(person?.relationClaims||[])])];
    const loadedClaims=new Set(this.chronicle?.data.claims.map(c=>c.id)||[]);
    ...
    return {sceneId:selected.id,itemId:selected.itemId,label:selected.label,setting:selected.setting,summary:selected.summary,narrative:selected.narrative,siteBackground:selected.siteBackground,role:person?.role||(selected.scenePlace?.settlement?.scope==='between-records'?'기록 사이 추정 배경':undefined),participants:selected.participants,place:selected.scenePlace?.label||selected.locationReference?.label,
      ...
      // #186: 항목 장면의 근거(위키백과 좌표 등)는 기본 사료 선택에 없을 수 있다. 기존 장면은 종전대로 근거를 요구하므로 안내하지 않는다.
      missingClaimsNote:selected.itemId&&missingClaimIds.length?`근거 ${missingClaimIds.length}건은 현재 선택한 사료 밖(위키백과 등)의 기록이라 이 화면에는 나오지 않습니다.`:'',
