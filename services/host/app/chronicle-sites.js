...
export function planHistoricalSites(data,packets,plan){
  ...
  for(const entityId of ['syj135-place-samnyeonsanseong','syj135-place-myeonghwalsanseong']){
    ...
    const summary=`${first.startYear}년부터 ${last.endYear}년까지 이어지는 기록 사이에 성곽을 보여줍니다. 계속 같은 모습으로 쓰였다는 확정 기록은 아닙니다. 마지막 표시 연도가 성을 버린 해를 뜻하지도 않습니다.`;
    sites.push({id:'background-'+entityId,entityId,kind:'event',year,label:'성곽 · '+label,
      ...
      scenePlace:{...first.place,label,coordinates:[first.place.lon,first.place.lat],displayBasis:'기록 사이를 잇는 추정 배경입니다. 성의 실제 윤곽이나 건물 배치를 복원한 모습은 아닙니다.'},
...
export function planContinuingCities(packets,plan,claims,settlementZones=[]){
  ...
  return chosen.map(({scene,place,claimIds})=>{
    ...
    return {id,entityId:id,kind:'event',year:plan.year,label,archetype:'settlement',
      ...
      summary:'이 위치의 도시 기록을 보고 이름 없는 생활 배경을 이어 보여줍니다. 이전 도시 이름과 행정 지위가 계속됐다는 뜻은 아닙니다. 사건과 인물의 기간을 늘린 것도 아닙니다. 건물과 거리 배치는 복원도가 아닙니다.\n규모는 축소 표현',
