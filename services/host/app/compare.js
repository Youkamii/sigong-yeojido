    ...
    try{
      ...
      for(const claim of data.rows){
        ...
        card.innerHTML=`<h3>${esc(claim.sourceLabel)}</h3><p>${esc(claim.chunk.locator||claim.citesChunk)}</p>
          ...
          ${claim.note?`<p class="empty">${esc(claim.note)}</p>`:''}
      ...
      for(const claim of data.links){
        ...
        card.innerHTML=`<p>${esc(claim.sourceLabel)} · ${claim.origin==='ai'?'AI 연결':'사람 연결'}</p><div class="quote">${esc(claim.quote)}</div>${claim.note?`<p class="empty">${esc(claim.note)}</p>`:''}<button class="card-btn">연결 주장과 원문</button>`;
