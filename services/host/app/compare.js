...
export class SourceComparison {
  constructor(host,callbacks){
    ...
    this.ready=fetch('/api/comparisons').then(r=>{if(!r.ok)throw new Error('비교 목록을 불러오지 못했습니다.');return r.json();}).then(data=>{
  ...
  async show(){
    ...
    rows.replaceChildren();links.replaceChildren();status.textContent='사료별 기록을 불러오고 있습니다…';
    try{
      await this.ready;if(seq!==this.sequence)return;
      const id=this.host.querySelector('select').value;
      if(!id){status.textContent='비교할 사례가 없습니다.';return;}
      ...
      if(!response.ok)throw new Error(data.error||'비교 결과를 불러오지 못했습니다.');
      this.render(data);
  ...
  async findDifferences(offset=0){
    ...
    this.host.querySelector('.compare-description').textContent='고른 사료에서 같은 사건의 연결과 연도 근거를 비교합니다. 연도를 서기로 바꾸지 못한 기록은 이 목록에 나오지 않습니다.';
    message.textContent='연결된 기록에서 연도 차이를 찾고 있습니다…';
    ...
    try{
      ...
      const data=await response.json();if(seq!==this.sequence)return;
      if(!response.ok)throw new Error(data.error||'연도 차이를 불러오지 못했습니다.');
      message.textContent=data.comparisons.length?`연도 차이가 있는 수록 사건 ${offset+1}~${offset+data.comparisons.length}`:'고른 사료에 연도 차이를 확인할 사건 연결이 없습니다.';
  ...
  render(data){
      ...
      status.textContent=data.rows.length?`${data.sourceCount}개 사료, ${data.rows.length}개 기록: ${data.differentRawDates?'원래 날짜 다름':'원래 날짜 같음'}${data.differentProjectedYears?', 서기로 바꾼 연도 다름':''}`:'고른 사료에 맞는 기록이 없습니다. 이 사례의 사료를 켜 보십시오.';
      for(const claim of data.rows){
        ...
        card.innerHTML=`<h3>${esc(claim.sourceLabel)}</h3><p>${esc(claim.chunk.locator||claim.citesChunk)}</p>
          ...
          <button class="card-btn" data-action="claim">기록 출처 보기</button><button class="card-btn" data-action="chunk">원문 보기</button><button class="card-btn" data-action="source">사료 설명 보기</button>`;
      ...
      if(!data.links.length){const empty=document.createElement('p');empty.className='empty';empty.textContent='고른 사료와 작성자에 맞는 사건 연결 근거가 없습니다.';links.append(empty);}
