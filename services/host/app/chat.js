export class EvidenceChat {
  constructor(host, callbacks) {
    ...
    host.innerHTML=`<h2>사료에 묻기</h2>
      <p class="chat-note">선택한 사료에 연결된 주장을 찾아 답한다. 각 문장의 출처를 누르면 인용과 원문을 볼 수 있다.</p>
  ...
  async ask() {
    ...
    this.pending=true;button.disabled=true;status.textContent='선택한 출처를 읽고 답변하는 중…';
    this.host.querySelector('.chat-answer').replaceChildren();
    try {
      ...
      status.textContent=result.status==='answered'?`주장 ${result.evidenceCount}개를 확인했다.${result.truncated?' 검색된 출처 중 일부만 사용했다.':''}`:'답할 출처가 부족하다.';
