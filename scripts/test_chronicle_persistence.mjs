...
const close={...event('close',.1),archetype:'court'};assets.activeScene='close';
assets.rebuild(plan([revised,close],1501));
assert.equal(assets.rows.find(r=>r.id==='a').compact,false,'Selecting a nearby event preserves the full city');
assert.equal(assets.rows.find(r=>r.id==='a').pick,original);assert.equal(disposed,0);
assert.ok(assets.rows.find(r=>r.id==='close').position.distanceTo(new THREE.Vector3(.1,0,0))>3,'The event gets a local stage beside the stable city');
...
assert.equal(disposed,1,'The retained city is disposed only after it leaves the plan');
...
for(const year of [1945,1970])assert.notEqual(key({...event(),year:year-1}),key({...event(),year}));
const jeju={...event(),scenePlace:{coordinates:[126.52194444444,33.509722222222]}};
assert.notEqual(key({...jeju,year:1954}),key({...jeju,year:1955}),'An urban display region entering the plan changes geometry identity');
assert.equal(key({...jeju,year:2005}),key({...jeju,year:2006}));
