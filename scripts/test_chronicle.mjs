...
assert.equal(contextAt(cityData,919).events[0].current,true,'The opening belongs to its own year');
assert.equal(contextAt(cityData,1000).events.length,0,'A city does not become a new event every year');
assert.equal(contextAt(cityData,1000).settings.length,1,'A supported city stays discoverable as a dated setting');
assert.equal(contextAt(cityData,918).settings.length,0);
assert.equal(contextAt(cityData,1232).settings.length,1);
assert.equal(contextAt(cityData,1233).settings.length,0,'The setting expires after its documented period');
...
assert.deepEqual(contextAt(places,1000).settings.map(e=>e.sceneId).sort(),['city-a','city-b'],'Settings at different places remain separately discoverable in the same year');
console.log('PASS: city openings, dated settings and expiry without repeated yearly events');
