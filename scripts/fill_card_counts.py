...
def build(source: str, sources_dir: Path = SOURCES) -> str:
    d = sources_dir / source
    groups, levels = collections.Counter(), collections.Counter()
    count = dated = empty = total_chars = 0
    for c in load(d / "chunks.jsonl"):
        count += 1
        groups[group_label(c["locator"], source)] += 1
        levels[c.get("level")] += 1
        dated += bool(c.get("date"))
        empty += not c.get("text")
        total_chars += c.get("charCount", 0)
    ann_t = collections.Counter(a.get("type") for a in load(d / "annotations.jsonl"))
    idx_t = collections.Counter(t.get("type") for t in load(d / "index-terms.jsonl"))

    lines = ["| 부·편 | chunk 수 |", "|---|---:|"]
    for g, n in groups.most_common():
        lines.append(f"| {g} | {fmt(n)} |")
    lines.append(f"| **합** | **{fmt(count)}** |")
    lines.append("")
    lines.append(
    ...
    lines.append(
        f"주석 {fmt(sum(ann_t.values()))}(" + " · ".join(f"{k} {fmt(v)}" for k, v in ann_t.most_common()) + "), "
        f"색인어 {fmt(sum(idx_t.values()))}(" + " · ".join(f"{k} {fmt(v)}" for k, v in idx_t.most_common()) + ")."
    )
    return "\n".join(lines)
...
def main(argv: list[str]) -> int:
    ...
    ap.add_argument("--sources-dir", type=Path, default=SOURCES)
    a = ap.parse_args(argv)
    card = a.sources_dir / f"{a.source}.md"
    text = io.open(card, encoding="utf-8").read()
    if START not in text or END not in text:
        print(f"{card}: 마커가 없다 ({START} … {END})")
        return 1
    block = START + "\n" + build(a.source, a.sources_dir) + "\n" + END
