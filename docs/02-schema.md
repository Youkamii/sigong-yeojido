...
지리적 불가능 검사는 아직 실행하지 않는다. 서로 다른 좌표나 같은 해의 두 장소만으로
사람의 이동이 불가능하다고 판단할 수 없다. 인물 동일성, 충분히 좁은 시각 범위, 위치 오차,
이동 수단·속도/거리의 출처가 함께 있는 사례가 먼저 필요하다. 그 근거와 실행 검증 전에는
이 항목을 완료로 세지 않는다. 현재 자동 검사의 통과는 역사 해석을 사람에게 검토받았다는 뜻이 아니다.
