# 인수인계 — 시공여지도 (2026-09-07, 읍·면·국가·사건 지도와 공개 원문 확대)
...
## 최신 진행 — Codex, 데이터 `a7c291ce` · 역로 표시 `24b02746`
...
- Source **1,072**, c2 chunk **2,601,030**, Git 추적 chunk **83,027**, Claim **9,416**.
  경계 레코드 연결 **9,028**, 나머지 역사 관계 **388**, 인용 chunk **9,240**, Location **230**.
  실제 Claim이 있는 Source **83**, 원문만 있는 Source **989**. 사람 작성·검토 완료 Claim은 0이다.
...
- Python **111개**·JavaScript **13개** 통과. [전체 TTL 검사](research/goal-build-check.json)는
  두 빌드 바이트 일치·rdflib 재파싱 **244,941 트리플**·참조 **38,241개**·digest·충돌 **6개**, 실패 0이다.
...
- 로컬/c2 TTL은 대용량 원문의 Source chunkCount 35개만 다르고 나머지 트리플은 같다.
  [전체 RDF 차이 대조](research/name-quotes-ttl-comparison-84-85.json)를 보존했다.
...
- [역로 선 표시](research/route-rendering-86.md)는 인공 시험 7개와 기존 국가 경계 9개 재검사를 통과했다.
  열린 선을 닫거나 끊어진 선을 잇지 않는다. 인공 자료는 운영에 넣지 않았고 실제 역사 선은 미확보다.
- 현재 배포 SHA·PID·RDF 수·TTL 해시·준비 시간은 [배포 기록](research/route-deployed-86.json)이 정본이다.
  FinBridge :8891과 `56aa0c3` 이후 판톨로지 이식 코어·vendor를 유지했다. 새 터미널 창은 열지 않았다.
- 개발·통합은 Codex, 조사·수집은 Claude Opus 5 / Max effort다. 구독 한도로 중단된 호출은 실패 상태와
  저장된 초안을 보존했다. 역로·진번 위치·미수록 공개 원문 조사 3건은 2026-09-07 06:51 KST부터
  숨김 실행하도록 로컬 `%TEMP%/sigong-goal-opus5/reset-manifest.json`에 예약했다. 예약을 조사 완료로 세지 않는다.

원래 §7은 **9개 완료·1개 부분·1개 결정 대기**다. 전체 이슈 #1~#86은 **77개 닫힘·9개 열림**이며,
열린 것은 #8 #12 #49 #51 #52 #56 #57 #62 #86이다. 모든 사료의 전문·직접 위치 근거·역로가 확보된 상태는 아니다.
