## 2026-09-12: 최신 사용자 요구사항과 미완 작업 인수인계 (#178)

**먼저 [요구사항·작업 인수인계](REQUIREMENTS_HANDOVER.md)를 읽는다.** 사용자가 반복해서 요구한 실제 거주 분포, 시대·지역별 상세한 3D 생활 세계, 인물·사건·시간·지리·성능·작업 방식과 완료 기준을 정리했다. 제공한 지도·UI 참고 이미지도 문서에서 열 수 있다.

미완 앱은 `codex/annual-world-fixes` / `b66dee50`에 푸시해 보존했다. 이 브랜치는 main 병합·서비스 배포하지 않았다. 마지막 자료 기반 생활 구역·원거리 모델 수정 이후의 실제 화면과 전 연도 브라우저 검사는 미완이며, 선사·서기 초 생활 구역 자료도 보강해야 한다. 새 문서의 현재 상태·다음 작업을 먼저 확인한 뒤 아래 과거 기록을 참고한다.

이번 인수인계 문서 전달은 앱 기능·역사적 완성도 검증 완료를 뜻하지 않는다.
