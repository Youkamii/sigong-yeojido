...
## 최신 진행 — Codex, 데이터·스캔 열람 `a2387fd0`, 운영 검증 `bcb9c375`
...
- Source **1,073**, c2 chunk **2,601,349**, Git 추적 chunk **83,346**, Claim **9,416**.
  경계 레코드 연결 **9,028**, 나머지 역사 관계 **388**, 인용 chunk **9,240**, Location **230**.
  실제 Claim이 있는 Source **83**, Claim이 없는 Source **990**. 사람 작성·검토 완료 Claim은 0이다.
...
- Python **111개**·JavaScript **14개** 통과. [전체 TTL 검사](research/goal-build-check.json)는
  두 빌드 바이트 일치·rdflib 재파싱 **244,954 트리플**·참조 **38,241개**·digest·충돌 **6개**, 실패 0이다.
...
- 로컬/c2 TTL은 대용량 원문의 Source chunkCount 35개만 다르고 나머지 트리플은 같다.
  [전체 RDF 차이 대조](research/ndl-scan-87-ttl-comparison.json)를 보존했다.
...
- 현재 배포 SHA·PID·RDF 수·TTL 해시·준비 시간은 [배포 기록](research/ndl-scan-87-deployed.json)이 정본이다.
...
원래 §7은 **9개 완료·1개 부분·1개 결정 대기**다. 전체 이슈 #1~#87은 **78개 닫힘·9개 열림**이며,
