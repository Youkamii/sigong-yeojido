...
7. **단군 완료·평양 부분(#35 #61)** — 한국민족문화대백과사전의 직접 설명을 인용해 壇君/檀君 `sameEntityAs` Claim을 추가했다. 엔티티는 합치지 않는다. 平穰/平壤의 직접 동일성은 보류한다. #61에서 지역·성곽 관계 6개를 추가했으며, 기존 검색 aliases를 동일성 근거로 쓰지 않는다.
...
curl -fsS http://127.0.0.1:8870/api/sources | python3 -c 'import json,sys; s=json.load(sys.stdin)["sources"]; print(len(s), sum(x["chunkCount"] for x in s))'  # 996 2567024
