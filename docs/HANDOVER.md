...
미검증: 패드 실기기(터치·로딩 시간·프레임). 로컬엔 Fuseki 가 없어 `/api/time`·`/api/chronicle` 이 503 이라 장면·인물은 안 보이는 상태에서 레이아웃만 확인했다(`?ui=full` 도 같은 503 — 이 변경과 무관). gstack 헤드리스 브라우저는 WebGL 부하로 가끔 재시작한다.
