...
국편 한국고대사료집성 중국편에 실린 『釋氏蒙求』의 한국사 관련 발췌다.
사서 전체 원문을 수록한 것으로 표시하지 않는다. 국편 사서 id `ko_080`를 Source 이름에 쓴다.
같은 한글 이름을 가진 다른 사서도 id가 달라 합쳐지지 않는다.

## 연도 근거와 한계

편찬 시점은 XML의 `source/dateIssued`에서 단일 연도나 정확한 날짜를 옮긴 값이다. 원표기: `청대(淸代)`.
...
이 범위는 사서 전체의 범위나 인물의 생몰년이 아니다. 저자 원표기 `영조(霊操)`도 XML의 서지 정보로만 보존한다.
표제는 `mainTitle`을 쓴다. 설명 요소의 name 속성에는 다른 책 이름을 복사한 흔적이 있어 표제로 쓰지 않는다.

## 국편 서지 설명

『釋氏蒙求』는 淸나라 때의 승려 霊操가 찬술한 책으로 漢나라 때 이래의 高僧들의 행적을 4言의 韻文으로 노래한 후 그 내용을 해설하고 있다. 上·下 2卷이며, 上卷 118句, 下卷 96句 등 모두 214句로 구성되어 있다. 4言의 운문을 사용한 것은 五代시대 後唐의 李翰이 편찬한 『蒙求』의 형식을 모방한 것으로 어린아이들이 외우기 쉽게 하고자 한 것이다. 韓国과 관련되는 내용은 上卷 중의 ‘元暁難敵’이라는 한 구절뿐인데, 원효가 신라에서 解行이 뛰어나 상대할만한 사람이 없었다고 이야기하고 있다.
...
chunk 가 놓인 층: level3 1. 연대(dateOccured) 붙은 chunk 0, 본문이 빈 chunk 0, 본문 글자 수 104.
주석 0(), 색인어 5(이름 3 · 서명 1 · 국명 1).
<!-- counts:end -->
