...
국편 한국고대사료집성 중국편에 실린 『華嚴懸談會玄記』의 한국사 관련 발췌다.
사서 전체 원문을 수록한 것으로 표시하지 않는다. 국편 사서 id `ko_079`를 Source 이름에 쓴다.
같은 한글 이름을 가진 다른 사서도 id가 달라 합쳐지지 않는다.

## 연도 근거와 한계

편찬 시점은 XML의 `source/dateIssued`에서 단일 연도나 정확한 날짜를 옮긴 값이다. 원표기: `원대(元代)`.
...
이 범위는 사서 전체의 범위나 인물의 생몰년이 아니다. 저자 원표기 `보서(普瑞)`도 XML의 서지 정보로만 보존한다.
표제는 `mainTitle`을 쓴다. 설명 요소의 name 속성에는 다른 책 이름을 복사한 흔적이 있어 표제로 쓰지 않는다.

## 국편 서지 설명

『華嚴懸談會玄記』는 元나라 때의 華嚴宗 승려 普瑞가 『華嚴經』의 사상을 해설한 저술로 40권으로 구성되어 있다. 이 책은 華厳宗의 第4祖로 일컬어지는 清涼澄観(738-839)의 『華厳玄談』에 주석을 붙여가며 해설하는 형식을 취하고 있다. 卷20에 新羅 元曉의 傳記가 언급되고 있다.
...
chunk 가 놓인 층: level3 1. 연대(dateOccured) 붙은 chunk 0, 본문이 빈 chunk 0, 본문 글자 수 943.
주석 1(원주 1), 색인어 46(이름 27 · 지명 9 · 서명 5 · 국명 5).
<!-- counts:end -->
