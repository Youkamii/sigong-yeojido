...
const source=(await readFile(new URL('../services/host/app/chronicle-paths.js',import.meta.url),'utf8')).replace("'three'",JSON.stringify(new URL('../services/host/vendor/three.module.min.js',import.meta.url).href)).replace(/'([.][/]scenery-[^']+)'/g,(_,path)=>JSON.stringify(new URL('../services/host/app/'+path,import.meta.url).href));
